...
def learn(env,
    ...
    for t in range(total_timesteps):
        t_eff = max(0, t - learning_starts)
        eps = exploration.value(t_eff)
...
#                    break
        try:
            new_raw, total_rew, done, _ = env.step(env_action)
            episode_rewards[-1] += total_rew
        except RuntimeError as e:
            if "time-out" in str(e):
                print("CARLA timeout detected — saving checkpoint and exiting.")
                save_checkpoint(
                    f"checkpoints/{model_identifier}_crash.pt",
                    policy_net,
                    target_net,
                    optimizer,
                    t,
        ...
        if t > 0 and t % 5000 == 0:
            save_checkpoint(
                f"checkpoints/{model_identifier}.pt",
                policy_net,
                target_net,
                optimizer,
                t,
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            loss = perform_qlearning_step(policy_net, target_net, optimizer, replay_buffer, batch_size, gamma, device, double_dqn=double_dqn)
